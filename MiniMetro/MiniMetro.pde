...
void draw(){
  ...
  if(screen == -4){
    ...
    text("Hence, it's important you distribute your lines so that you cover all three types of stations!", width/20+20, height/10+90);
    text("When a passenger waits for too long at a station, you start to crowd!", width/20+20, height/10+130);
    text("Similarly, when a station is full, you also begin to crowd!", width/20+20, height/10+170);
    text("Stations tell you the number of passengers of each type directly.", width/20+20, height/10+510);
    text("Trains also tell you what passengers it is carrying by symbols.", width/20+20, height/10+550);
    
    textSize(14);
    text("This is a train.", 4*width/5-50, height/10+420);
    
    fill(255);
    circle(width/3-30, height/2+20, 240);
    textSize(60);
    fill(0);
    text("C:2", width/3-65, height/2-40);
    text("T:1", width/3-65, height/2+30);
    text("S:1", width/3-65, height/2+100);
    fill(255);
    rect(2*width/3+120, height/2+20, 120, 120, 30, 30, 30, 30);
    fill(0);
    circle(2*width/3+30, height/2-50, 30);
    square(2*width/3-10, height/2-50, 30);
    circle(2*width/3-50, height/2-50, 30);
    triangle(2*width/3-100, height/2-35, 2*width/3-70, height/2-35,2*width/3-85, height/2-65);
